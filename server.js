...
olServerId: { type: String, default: null },
  },
  { timestamps: true }
);
...
const couponSchema = new mongoose.Schema(
  {
    ...
    assignedToVpsId: { type: String, default: null },
  },
  { timestamps: true }
);
...
async function checkCouponValidity(code, price, userId, category, userEmail, userRole, vpsId) {
  if (!code) {
  ...
  if (coupon.assignedToEmail) {
    ...
    if (coupon.assignedToVpsId) {
    if (!vpsId || coupon.assignedToVpsId !== vpsId) {
      return { valid: false, message: "Ye coupon code valid nahi hai." };
    }
  }
...
app.post("/api/coupons/validate", couponLimiter, protect, async (req, res) => {
  try {
    ...
    const result = await checkCouponValidity(code, effectivePrice, req.userId, cat, currentUser?.email, currentUser?.role, vpsId);
...
app.post("/api/vps/create-payment", protect, async (req, res) => {
  try {
    ...
    if (couponCode) {
      const result = await checkCouponValidity(couponCode, effectivePrice, req.userId, cat, user?.email, user?.role, plan.vpsId);
      if (!result.valid) {
...
// ==================== END METRICSX ROUTES ====================
// ==================== OCEANLINUX CLIENT (NAYA) ====================
const OL_BASE = process.env.OCEANLINUX_BASE || "https://oceanlinux.com/api/v1";
...
async function olFindOwned(userId, olServerId) {
  return Order.findOne({ user: userId, olServerId: String(olServerId) });
}
// ==================== END OCEANLINUX CLIENT ====================
// ==================== OCEANLINUX ROUTES ====================

// ---- Admin: purchase karo naya server ----
app.post("/api/ol/admin/purchase", protect, isAdmin, async (req, res) => {
  try {
    const { email, serverId, memory, duration, os, hostname, category } = req.body;
    if (!email || !serverId || !memory) {
      return res.status(400).json({ message: "email, serverId, memory zaroori hai." });
    }

    const targetUser = await User.findOne({ email: String(email).toLowerCase().trim() });
    if (!targetUser) return res.status(404).json({ message: "User nahi mila." });

    const purchase = await olApi("/servers/purchase", "POST", {
      serverId,
      memory,
      duration: duration || 1,
      specifications: { os: os || "Ubuntu 22", hostname: hostname || `sb-${Date.now()}` },
    });

    const order = await Order.create({
      user: targetUser._id,
      planName: purchase.planName || serverId,
      category: category === "linux" ? "linux" : "vps",
      vpsId: `oceanlinux_${serverId}_${Date.now()}`,
      nameOrIp: purchase.ip || serverId,
      ram: memory,
      price: purchase.price || 0,
      finalAmount: purchase.price || 0,
      status: "delivered",
      paymentGateway: "manual",
      paymentStatus: "paid",
      olServerId: String(purchase.serverId || purchase._id || purchase.id),
      deliveryIp: purchase.ip || "",
      deliveryOS: os || "Ubuntu 22",
      deliveredAt: new Date(),
    });

    try { sendTelegramMessage(buildOrderAlertMessage({ user: targetUser, order, isManual: true })); } catch (e) {}

    res.status(201).json({ message: "OceanLinux server purchase ho gaya.", order });
  } catch (err) {
    res.status(500).json({ message: "Purchase fail hua.", error: err.message });
  }
});

// ---- User: sirf apne OceanLinux servers dekho ----
app.get("/api/ol/my-servers", protect, async (req, res) => {
  try {
    const orders = await Order.find({ user: req.userId, olServerId: { $ne: null } });
    if (!orders.length) return res.json({ success: true, servers: [] });

    const allServers = await olApi("/servers");
    const ownedIds = orders.map((o) => String(o.olServerId));
    const servers = (allServers.servers || allServers.data || [])
      .filter((s) => ownedIds.includes(String(s.id || s._id || s.serverId)))
      .map((s) => {
        const o = orders.find((x) => String(x.olServerId) === String(s.id || s._id || s.serverId));
        return { ...s, orderId: o._id, planName: o.nameOrIp || o.planName };
      });
...
// ---- Admin: existing order ko OceanLinux server se link karo ----
app.put("/api/ol/admin/link/:orderId", protect, isAdmin, async (req, res) => {
  try {
    const { olServerId } = req.body;
    if (!olServerId) return res.status(400).json({ message: "olServerId zaroori hai." });
    const order = await Order.findByIdAndUpdate(req.params.orderId, { olServerId: String(olServerId) }, { new: true });
    if (!order) return res.status(404).json({ message: "Order nahi mila." });
    res.json({ message: "OceanLinux server link ho gaya.", order });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
...
app.get("/api/ol/admin/live-vms", protect, isAdmin, async (req, res) => {
  try {
    ...
    const vms = list.map((v) => ({
      vmId: v.id || v._id || v.serverId,
      ip: v.ip || v.ipAddress || "",
      os: v.os || "",
      plan: v.plan || v.planName || "",
      status: v.status || v.liveState || "",
    }));
    res.json({ success: true, vms });
  } catch (err) {
    res.json({ success: false, message: err.message });
...
// ---- Admin: user ka sabse naya order OceanLinux server se link karo (Manual Order ke liye) ----
app.post("/api/ol/admin/link-latest", protect, isAdmin, async (req, res) => {
  try {
    const { olServerId, email } = req.body;
    const cleanEmail = String(email || "").toLowerCase().trim();
    if (!olServerId || !cleanEmail) {
      return res.status(400).json({ message: "email aur olServerId zaroori hai." });
    }
    const user = await User.findOne({ email: cleanEmail });
    if (!user) return res.status(404).json({ message: "User nahi mila." });
    const order = await Order.findOne({ user: user._id }).sort({ createdAt: -1 });
    if (!order) return res.status(404).json({ message: "Order nahi mila." });
    order.olServerId = String(olServerId);
    await order.save();
    res.json({ message: "OceanLinux server link ho gaya.", order });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
...
app.post("/api/admin/coupons", protect, isAdmin, async (req, res) => {
  try {
    const {
      ...
      category, assignedToEmail, allowedRole, assignedToVpsId,
    } = req.body;
    ...
    const coupon = await Coupon.create({
      ...
      assignedToVpsId: assignedToVpsId || null,
    });
...
app.put("/api/admin/coupons/:id", protect, isAdmin, async (req, res) => {
  try {
    ...
    if (assignedToVpsId !== undefined) updateFields.assignedToVpsId = assignedToVpsId || null;
