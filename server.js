...
// ==================== PROXY LIVE CHECKER (STATELESS, credentials never stored) ====================
const { HttpsProxyAgent } = require("https-proxy-agent");

const PROXY_CHECK_SITES = {
  irctc:     { label: "IRCTC",     url: "https://www.irctc.co.in" },
  ...
  microsoft: { label: "Microsoft", url: "https://www.microsoft.com" },
  amazon:    { label: "Amazon",    url: "https://www.amazon.in" },
...
function parseProxyLine(line) {
  ...
  const [ip, port, username, ...rest] = parts;
...
const proxyCheckLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 20,
  message: { message: "Bahut zyada proxy check attempts. 15 minute baad try karo." },
});

app.post("/api/proxy/check-live", protect, proxyCheckLimiter, async (req, res) => {
  try {
    const { proxyLine, siteKeys } = req.body;

    const proxy = parseProxyLine(proxyLine);
    if (!proxy) {
      return res.status(400).json({ message: "Proxy format galat hai. Sahi format: ip:port:username:password" });
    }

    const keys = Array.isArray(siteKeys) && siteKeys.length > 0 ? siteKeys : ["irctc"];
    const validKeys = keys.filter((k) => PROXY_CHECK_SITES[k]);
    if (validKeys.length === 0) {
      return res.status(400).json({ message: "Kam se kam ek valid website select karo." });
    }

    const results = await Promise.all(
      validKeys.map(async (key) => {
        const site = PROXY_CHECK_SITES[key];
        const result = await checkOneSite(proxy, site.url);
        return { key, label: site.label, ...result };
      })
    );

    const workingCount = results.filter((r) => r.working).length;
    const bestLatency = results
      .filter((r) => r.working)
      .reduce((min, r) => (r.latencyMs < min ? r.latencyMs : min), Infinity);

    res.json({
      ip: proxy.ip,
      port: proxy.port,
      username: proxy.username,
      overallWorking: workingCount > 0,
      workingCount,
      totalCount: results.length,
      scorePercent: Math.round((workingCount / results.length) * 100),
      bestLatencyMs: isFinite(bestLatency) ? bestLatency : null,
      results,
    });
  } catch (err) {
    res.status(500).json({ message: "Proxy check karte waqt error", error: err.message });
  }
});
// ==================== END PROXY LIVE CHECKER ====================
