...
// Software Admin Credentials - ONLY from .env
const SOFTWARE_ADMIN_EMAIL = process.env.SOFTWARE_ADMIN_EMAIL;
const SOFTWARE_ADMIN_PASSWORD = process.env.SOFTWARE_ADMIN_PASSWORD;

// Agar .env me nahi hai to error throw karo
if (!SOFTWARE_ADMIN_EMAIL || !SOFTWARE_ADMIN_PASSWORD) {
  console.error("❌ SOFTWARE_ADMIN_EMAIL and SOFTWARE_ADMIN_PASSWORD must be set in .env");
  process.exit(1);
}
...
const softwareOrderSchema = new mongoose.Schema(
  {
    user: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true },
...
const SoftwareOrder = mongoose.model("SoftwareOrder", softwareOrderSchema);
...
// ==================== SOFTWARE ADMIN MIDDLEWARE ====================
const isSoftwareAdmin = (req, res, next) => {
  const authHeader = req.headers.authorization;
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
    if (decoded.role !== "software_admin") {
      return res.status(403).json({ message: "Access denied. Software Admin only." });
    }
...
app.post("/api/software-admin/login", async (req, res) => {
  try {
    const { email, password } = req.body;
    ...
    if (email.toLowerCase() !== SOFTWARE_ADMIN_EMAIL.toLowerCase()) {
      return res.status(401).json({ message: "Invalid credentials" });
    }

    if (password !== SOFTWARE_ADMIN_PASSWORD) {
      return res.status(401).json({ message: "Invalid credentials" });
    }
    ...
    res.json({
      message: "Software Admin Login successful!",
      token,
      admin: {
        email: SOFTWARE_ADMIN_EMAIL,
        role: "software_admin"
...
app.get("/api/software", async (req, res) => {
  try {
    const software = await Software.find({ available: true }).sort({ createdAt: -1 });
    res.json(software);
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
app.get("/api/software/:id", async (req, res) => {
  try {
    ...
    if (!software) {
      return res.status(404).json({ message: "Software nahi mila." });
    }
    res.json(software);
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
...
app.post("/api/software/create-order", protect, async (req, res) => {
  try {
    ...
    if (!software.available) {
      return res.status(400).json({ message: "Ye software abhi available nahi hai." });
    }

    const existingOrder = await SoftwareOrder.findOne({
      user: req.userId,
      softwareId: softwareId,
      status: { $in: ["paid", "delivered"] }
    });
    if (existingOrder) {
      return res.status(400).json({ message: "Aap ye software pehle hi khareed chuke hain." });
    }

    const order = await SoftwareOrder.create({
      user: req.userId,
    ...
    res.json({
      message: "Order create ho gaya! UPI se payment karo.",
      orderId: order._id,
      softwareName: software.name,
...
app.post("/api/software/verify-payment", protect, async (req, res) => {
  try {
    const { orderId, transactionId } = req.body;

    if (!orderId || !transactionId) {
      return res.status(400).json({ message: "Order ID aur Transaction ID dono bharo." });
    }

    const order = await SoftwareOrder.findOne({
      _id: orderId,
      user: req.userId
    });
    ...
    if (order.upiPaymentStatus === "paid") {
      return res.status(400).json({ message: "Ye order pehle se paid hai." });
    }

    order.upiTransactionId = transactionId;
    order.upiPaymentStatus = "pending_verification";
    ...
    try {
      const user = await User.findById(req.userId);
      const msg = [
        `🛒 <b>Naya Software Order (UPI)</b>`,
        ``,
        `📦 <b>Software:</b> ${order.softwareName}`,
        `💰 <b>Amount:</b> ₹${order.price}`,
        `👤 <b>Buyer:</b> ${user?.name || "Unknown"}`,
        `📧 <b>Email:</b> ${user?.email || "Unknown"}`,
        `🧾 <b>Transaction ID:</b> <code>${transactionId}</code>`,
        `🆔 <b>Order ID:</b> <code>${order._id}</code>`,
        ``,
        `⚠️ <b>Admin Action Required:</b> Verify payment and deliver software.`
      ].join("\n");
      sendTelegramMessage(msg);
    } catch (alertErr) {
      console.error("Telegram alert error:", alertErr.message);
    ...
    res.json({
      message: "Payment details submit ho gaye! Admin verify karke software deliver karega."
    });
...
// ==================== SOFTWARE ADMIN ROUTES ====================
app.get("/api/admin/software", isSoftwareAdmin, async (req, res) => {
  try {
...
app.post("/api/admin/software", isSoftwareAdmin, async (req, res) => {
  try {
    const { name, description, price, image, version, fileUrl, systemRequirements } = req.body;

    if (!name || !description || !price) {
      return res.status(400).json({ message: "Name, description aur price zaroori hai." });
    }
...
app.put("/api/admin/software/:id", isSoftwareAdmin, async (req, res) => {
  try {
    const { name, description, price, image, version, fileUrl, systemRequirements, available } = req.body;
    ...
    if (systemRequirements !== undefined) updateFields.systemRequirements = systemRequirements;
    if (available !== undefined) updateFields.available = available;
    ...
    res.json({ message: "Software update ho gaya!", software });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
});

app.delete("/api/admin/software/:id", isSoftwareAdmin, async (req, res) => {
  try {
    const software = await Software.findByIdAndDelete(req.params.id);
    if (!software) {
      return res.status(404).json({ message: "Software nahi mila." });
    }
    res.json({ message: "Software delete ho gaya." });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
});

app.get("/api/admin/software-orders", isSoftwareAdmin, async (req, res) => {
  try {
    const orders = await SoftwareOrder.find()
      .populate("user", "name email phone")
...
app.put("/api/admin/software-orders/:id", isSoftwareAdmin, async (req, res) => {
  try {
    ...
    const order = await SoftwareOrder.findByIdAndUpdate(
      req.params.id,
      updateFields,
      { new: true }
    );
    ...
    if (status === "delivered") {
      try {
        ...
        const msg = [
          `✅ <b>Software Delivered!</b>`,
          ``,
          `📦 <b>Software:</b> ${order.softwareName}`,
          `👤 <b>User:</b> ${user?.name || "Unknown"}`,
          `📧 <b>Email:</b> ${user?.email || "Unknown"}`,
          `🔗 <b>Download:</b> ${deliveryFileUrl || "Contact admin for download link"}`,
          `📝 <b>Notes:</b> ${deliveryNotes || "None"}`,
        ].join("\n");
        sendTelegramMessage(msg);
      } catch (alertErr) {
        console.error("Telegram alert error:", alertErr.message);
    ...
    res.json({ message: "Software order update ho gaya!", order });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
...
// ==================== UPI QR CODE GENERATOR (NO RAZORPAY) ====================
app.get("/api/upi/generate-qr", async (req, res) => {
  try {
    ...
    let upiLink = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(name)}&cu=INR`;
    if (amount && Number(amount) > 0) {
      upiLink += `&am=${amount}`;
    }
    
    // FREE QR API - QR code generate karo
    ...
    res.status(500).json({ 
      success: false, 
      message: "QR generate nahi ho paya", 
      error: err.message 
    });
  }
});

// ==================== UPI PAYMENT VERIFY (MANUAL) ====================
app.post("/api/upi/verify-payment", protect, async (req, res) => {
  try {
    const { orderId, transactionId } = req.body;
    ...
    // Telegram alert admin ko
    try {
      const user = await User.findById(req.userId);
      const msg = [
        `🛒 <b>Naya UPI Payment - Software Order</b>`,
        ``,
        `📦 <b>Software:</b> ${order.softwareName}`,
        `💰 <b>Amount:</b> ₹${order.price}`,
        `👤 <b>Buyer:</b> ${user?.name || "Unknown"}`,
    ...
    res.json({
      message: "Payment details submit ho gaye! Admin verify karega.",
      success: true
    });
